...
public class $2ObjectOrientedPrograming {
    ...
    // (3) 다형성이란 어떤 객체의 속성이나 기능이 상황에 따라 여러 가지 형태를 가질 수 있는 성질을 의미한다.
    // ex) 연극의 배역이 있고, 배우는 변경될 수 있다.

    // (4) 캡슐화란 클래스 안에 서로 연관있는 속성과 기능들을 하나의 캡슐(capsule)로 만들어 데이터를 외부로부터 보호하는 것을 말한다.
    // 캡슐화로 얻을 수 있는 이점으로는 1)데이터 보호(data protection) – 외부로부터 클래스에 정의된 속성과 기능들을 보호
    // 2)데이터 은닉(data hiding) – 내부의 동작을 감추고 외부에는 필요한 부분만 노출 이 있다.
    // 캡슐화를 하기 위해 접근제어자를 활용하거나 getter/setter를 이용한다.
